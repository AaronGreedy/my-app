'use client';

import { p, fmtItDate } from '@/lib/design';
import { useAuth } from '@/lib/auth-context';
import { useTodos, useCountdowns, useNotes, daysUntil } from '@/lib/user-store';

// Pannello laterale destro della Home su desktop. Riempie lo spazio a destra
// con DATI REALI già presenti nell'app: to-do aperti, countdown imminenti e
// note recenti. Niente dati finti: legge gli stessi store della Home.
//
// Le liste usano una griglia auto-fill: le card riempiono tutta la larghezza
// del pannello (1 colonna se stretto, 2+ se largo) → niente spazio vuoto.
export function HomePanel({ onNavigate }: { onNavigate?: (s: 'home'|'cal'|'brain'|'me'|'focus'|'nova'|'settings') => void }) {
  const { user } = useAuth();
  const uid = user?.uid ?? null;
  const { todos, toggleTodo } = useTodos(uid);
  const { countdowns } = useCountdowns(uid);
  const { notes } = useNotes(uid);

  // To-do ancora aperti, priorità alta prima, poi più vecchi prima.
  const open = todos
    .filter(t => !t.done)
    .sort((a, b) => (b.priority - a.priority) || (a.createdAt - b.createdAt))
    .slice(0, 12);

  // Countdown non conclusi, il più vicino prima.
  const next = countdowns
    .filter(c => !c.done)
    .map(c => ({ ...c, days: daysUntil(c.date) }))
    .sort((a, b) => a.days - b.days)
    .slice(0, 6);

  // Note più recenti (già ordinate per createdAt desc dallo store).
  const recent = notes.slice(0, 6);

  // griglia che riempie: 1 colonna se il pannello è stretto, 2+ se largo
  const grid: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: 8 };
  const card: React.CSSProperties = { padding: '11px 13px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' };

  const Label = ({ children, count, onAll }: { children: React.ReactNode; count?: number; onAll?: () => void }) => (
    <div style={{ display:'flex', alignItems:'center', gap:8, fontFamily:p.monoFont, fontSize:9.5, letterSpacing:0.22, color:p.dim, textTransform:'uppercase', margin:'0 2px 10px' }}>
      <span>{children}</span>
      {count !== undefined && <span style={{ color:p.muted }}>· {count}</span>}
      <span style={{ flex:1 }} />
      {onAll && <span onClick={onAll} style={{ color:p.muted, cursor:'pointer' }}>vedi tutto →</span>}
    </div>
  );

  return (
    <div style={{ padding: '24px 20px 60px', width: '100%' }}>

      {/* ── TO-DO APERTI ──────────────────────────────────────────────── */}
      <Label count={open.length} onAll={() => onNavigate?.('brain')}>Da fare</Label>
      {open.length === 0 ? (
        <div style={{ ...card, fontFamily:p.monoFont, fontSize:11, color:p.dim, lineHeight:1.5, marginBottom:30 }}>
          Niente in sospeso. Cattura qualcosa col + a sinistra.
        </div>
      ) : (
        <div style={{ ...grid, marginBottom:30 }}>
          {open.map(t => (
            <div key={t.id} onClick={() => toggleTodo(t.id)} style={{ ...card, display:'flex', alignItems:'flex-start', gap:10, cursor:'pointer' }}>
              {/* checkbox vuota: tap = fatto */}
              <div style={{ width:16, height:16, borderRadius:5, flexShrink:0, marginTop:1, border:`1.5px solid ${p.muted}` }} />
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontFamily:p.bodyFont, fontSize:13, color:p.fg, lineHeight:1.3 }}>{t.text}</div>
                <div style={{ fontFamily:p.monoFont, fontSize:9, color: t.priority === 3 ? p.red : t.priority === 2 ? p.orange : p.dim, textTransform:'uppercase', letterSpacing:0.15, marginTop:3 }}>
                  {'!'.repeat(t.priority)}{t.dueDate ? ` · ${fmtItDate(t.dueDate)}` : ''}
                </div>
              </div>
            </div>
          ))}
        </div>
      )}

      {/* ── COUNTDOWN IMMINENTI ───────────────────────────────────────── */}
      <Label count={next.length}>Countdown</Label>
      {next.length === 0 ? (
        <div style={{ ...card, fontFamily:p.monoFont, fontSize:11, color:p.dim, marginBottom:30 }}>Nessun countdown.</div>
      ) : (
        <div style={{ ...grid, marginBottom:30 }}>
          {next.map(c => (
            <div key={c.id} style={{ ...card, display:'flex', alignItems:'center', gap:12 }}>
              <div style={{ fontFamily:p.displayFont, fontSize:26, fontWeight:800, color:p.orange, lineHeight:1, minWidth:42 }}>
                {c.days}<span style={{ fontSize:10, color:p.muted, fontFamily:p.monoFont }}>g</span>
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontFamily:p.bodyFont, fontSize:12.5, fontWeight:700, color:p.fg, textTransform:'uppercase', letterSpacing:-0.1 }}>{c.label}</div>
                <div style={{ fontFamily:p.monoFont, fontSize:9, color:p.muted, marginTop:2 }}>{fmtItDate(c.date)}{c.note ? ` · ${c.note}` : ''}</div>
              </div>
            </div>
          ))}
        </div>
      )}

      {/* ── NOTE RECENTI ──────────────────────────────────────────────── */}
      <Label count={notes.length} onAll={() => onNavigate?.('brain')}>Note recenti</Label>
      {recent.length === 0 ? (
        <div style={{ ...card, fontFamily:p.monoFont, fontSize:11, color:p.dim }}>Nessuna nota ancora.</div>
      ) : (
        <div style={grid}>
          {recent.map(n => (
            <div key={n.id} onClick={() => onNavigate?.('brain')} style={{ ...card, cursor:'pointer' }}>
              <div style={{ fontFamily:p.bodyFont, fontSize:13, fontWeight:600, color:p.fg, lineHeight:1.3, overflow:'hidden', textOverflow:'ellipsis', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' as React.CSSProperties['WebkitBoxOrient'] }}>{n.title || n.body}</div>
              {n.tags.length > 0 && (
                <div style={{ display:'flex', gap:5, flexWrap:'wrap', marginTop:7 }}>
                  {n.tags.slice(0, 3).map(tag => (
                    <span key={tag} style={{ fontFamily:p.monoFont, fontSize:8.5, color:p.cyan, textTransform:'uppercase', letterSpacing:0.1, padding:'2px 7px', borderRadius:99, border:`1px solid rgba(0,240,255,0.3)` }}>{tag}</span>
                  ))}
                </div>
              )}
            </div>
          ))}
        </div>
      )}

    </div>
  );
}
