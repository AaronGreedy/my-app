...
interface Holiday { label: string; type: 'fest'|'religious'|'celebr'|'sunday' }
...
function italianHolidays(year: number): Map<string, Holiday> {
  ...
  add(new Date(year, 0, 1),   { label: 'Capodanno',         type:'fest' });
  add(new Date(year, 0, 6),   { label: 'Epifania',          type:'religious' });
  add(new Date(year, 1, 14),  { label: 'San Valentino',     type:'celebr' });
  add(new Date(year, 2, 19),  { label: 'Festa del Papà',    type:'celebr' });
  add(easter,                  { label: 'Pasqua',            type:'religious' });
  add(easterMon,               { label: 'Pasquetta',         type:'religious' });
  add(new Date(year, 3, 25),  { label: 'Liberazione',       type:'fest' });
  add(new Date(year, 4, 1),   { label: 'Festa del Lavoro',  type:'fest' });
  add(mothersDay,              { label: 'Festa della Mamma',type:'celebr' });
  add(new Date(year, 5, 2),   { label: 'Festa Repubblica',  type:'fest' });
  add(new Date(year, 7, 15),  { label: 'Ferragosto',        type:'fest' });
  add(new Date(year, 9, 31),  { label: 'Halloween',         type:'celebr' });
  add(new Date(year, 10, 1),  { label: 'Ognissanti',        type:'religious' });
  add(new Date(year, 11, 8),  { label: 'Immacolata',        type:'religious' });
  add(new Date(year, 11, 25), { label: 'Natale',            type:'religious' });
  add(new Date(year, 11, 26), { label: 'Santo Stefano',     type:'religious' });
  add(new Date(year, 11, 31), { label: 'San Silvestro',     type:'celebr' });
  return map;
}
...
const SUNDAY_HOLIDAY: Holiday = { label: 'Domenica', type: 'sunday' };
...
export function CalendarScreen() {
  ...
  // Streak allenamento rimosso 2026-05-23 — Aaron: "non so se serve li".
  // Resta come funzione disponibile se vorremo riusarlo (es. trofei).
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflowY: 'auto', overflowX: 'hidden', background: p.bg, color: p.fg, fontFamily: p.bodyFont }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, padding: 'calc(env(safe-area-inset-top, 0px) + 14px) 18px calc(env(safe-area-inset-bottom, 0px) + 130px)' }}>
        ...
        {/* Card "STREAK ALLENAMENTO" rimossa 2026-05-23 (Aaron: "non so se
            serve li"). Lo streak vero si vedrà altrove in futuro (heatmap
            in Me → Habits già lo copre). */}
                    ...
                    <button key={k} onClick={() => { setSelDay(d.getDate()); if (d.getMonth() !== vm || d.getFullYear() !== vy) { setVm(d.getMonth()); setVy(d.getFullYear()); } }} style={{
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ display:'flex', gap:8, flexWrap:'wrap', fontFamily:p.monoFont, fontSize:10, color:p.muted }}>
                          {hasFit  && <span style={{ color:p.orange }}>fit · {(dd?.workouts ?? []).join('+')}</span>}
            ...
            <NeonGlass style={{ marginTop:14 }} tint={holiday ? `${HOLIDAY_COLOR[holiday.type]}11` : 'rgba(255,255,255,0.04)'} edge={holiday ? `${HOLIDAY_COLOR[holiday.type]}55` : undefined} radius={20}>
              <div style={{ padding:'14px 16px' }}>
                ...
                <div style={{ marginTop:6, fontFamily:p.bodyFont, fontSize:14, color:dayWorkouts.length > 0 ? p.orange : p.muted }}>
                  {dayWorkouts.length > 0 ? `fit · ${dayWorkouts.join(' + ')}` : ''}
                </div>
                {dayMood && (
